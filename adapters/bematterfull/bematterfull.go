...
func (a *adapter) buildEndpointFromRequest(imp *openrtb2.Imp) (string, error) {
	...
	var bematterfullExt openrtb_ext.ExtBematterfull
	if err := jsonutil.Unmarshal(impExt.Bidder, &bematterfullExt); err != nil {
		return "", &errortypes.BadInput{
			Message: fmt.Sprintf("Failed to deserialize Bematterfull extension: %v", err),
		}
	}

	endpointParams := macros.EndpointTemplateParams{
		Host:     bematterfullExt.Env,
		SourceId: bematterfullExt.Pid,
	}
...
func (a *adapter) MakeBids(openRTBRequest *openrtb2.BidRequest, requestToBidder *adapters.RequestData, bidderRawResponse *adapters.ResponseData) (*adapters.BidderResponse, []error) {
	...
	if bidderRawResponse.StatusCode == http.StatusServiceUnavailable {
		return nil, []error{&errortypes.BadInput{
			Message: "Bidder Bematterfull is unavailable. Please contact the bidder support.",
		}}
	}
